...
class Residual(torch.nn.Module):
    ...
    def __init__(self, in_ch, out_ch, s, r, dp_rate=0, fused=True):
        ...
        if fused:
            features = [
                Conv(in_ch, r * in_ch, activation=torch.nn.SiLU(), k=3, s=s),
                Conv(r * in_ch, out_ch, identity) if r != 1 else identity,
                DropPath(dp_rate) if self.add else identity,
            ]
        else:
            features = [
                Conv(in_ch, r * in_ch, torch.nn.SiLU()) if r != 1 else identity,
    ...
    def __init__(self, drop_rate=0, num_class=1000):
        ...
        for i in range(num_dep[0]):
            if i == 0:
                ...
                self.p1.append(
                    Residual(filters[0], filters[0], 1, 1, dp_rates[dp_index])
                )
            else:
                self.p1.append(
                    Residual(filters[0], filters[0], 1, 1, dp_rates[dp_index])
        ...
        for i in range(num_dep[1]):
            if i == 0:
                self.p2.append(
                    Residual(filters[0], filters[1], 2, 4, dp_rates[dp_index])
                )
            else:
                self.p2.append(
                    Residual(filters[1], filters[1], 1, 4, dp_rates[dp_index])
                )
            dp_index += 1
        # p3/8
        for i in range(num_dep[2]):
            if i == 0:
                self.p3.append(
                    Residual(filters[1], filters[2], 2, 4, dp_rates[dp_index])
                )
            else:
                self.p3.append(
                    Residual(filters[2], filters[2], 1, 4, dp_rates[dp_index])
                )
            dp_index += 1
        # p4/16
        for i in range(num_dep[3]):
            if i == 0:
                self.p4.append(
                    Residual(filters[2], filters[3], 2, 4, dp_rates[dp_index], False)
                )
            else:
                self.p4.append(
                    Residual(filters[3], filters[3], 1, 4, dp_rates[dp_index], False)
                )
            dp_index += 1
        for i in range(num_dep[4]):
            if i == 0:
                self.p4.append(
                    Residual(filters[3], filters[4], 1, 6, dp_rates[dp_index], False)
                )
            else:
                self.p4.append(
                    Residual(filters[4], filters[4], 1, 6, dp_rates[dp_index], False)
                )
            dp_index += 1
        # p5/32
        for i in range(num_dep[5]):
            if i == 0:
                self.p5.append(
                    Residual(filters[4], filters[5], 2, 6, dp_rates[dp_index], False)
                )
            else:
                self.p5.append(
                    Residual(filters[5], filters[5], 1, 6, dp_rates[dp_index], False)
                )
            dp_index += 1
        for i in range(num_dep[6]):
            if i == 0:
                self.p5.append(
                    Residual(filters[5], filters[6], 2, 6, dp_rates[dp_index], False)
                )
            else:
                self.p5.append(
                    Residual(filters[6], filters[6], 1, 6, dp_rates[dp_index], False)
                )
            dp_index += 1
        ...
        self.fc1 = torch.nn.Sequential(
            Conv(filters[6], filters[7], torch.nn.SiLU()),
            torch.nn.AdaptiveAvgPool2d(1),
            torch.nn.Flatten(),
        )
...
class EMA:
    ...
    def update(self, model):
        self.update_fn(model, fn=lambda e, m: self.decay * e + (1.0 - self.decay) * m)
...
class StepLR:
    def __init__(self, optimizer):
        ...
        self.base_values = [
            param_group["initial_lr"] for param_group in self.optimizer.param_groups
        ]
        ...
        self.warmup_steps = [
            (v - self.warmup_lr_init) / self.warmup_epochs for v in self.base_values
        ]
    ...
    def step(self, epoch: int) -> None:
        if epoch < self.warmup_epochs:
            ...
            values = [
                v * (self.decay_rate ** (epoch // self.decay_epochs))
    ...
    def step(self, closure=None):
        ...
        for param_group in self.param_groups:
            for param in param_group["params"]:
                ...
                one_minus_alpha = 1.0 - param_group["alpha"]

                state["step"] += 1

                if param_group["weight_decay"] != 0:
                    if (
                        "decoupled_decay" in param_group
                        and param_group["decoupled_decay"]
                    ):
                ...
                if param_group["momentum"] > 0:
                    ...
                    if (
                        "lr_in_momentum" in param_group
                        and param_group["lr_in_momentum"]
                    ):
                        buf.mul_(param_group["momentum"]).addcdiv_(
                            grad, avg, value=param_group["lr"]
                        )
                        param.data.add_(-buf)
                    else:
